...
function renderizarMetricasAdmin() {
  ...
  const metricaHorario = horarioMasCargado
    ...
    : {
        valor: "0",
        ayuda: "Todavía no hay reservas cargadas."
      };
...
function obtenerEstadoPagoCliente(dni, clienteDatos = null) {
  ...
  return {
    texto: "Pago completo",
    clase: "bg-success-subtle text-success-emphasis",
    detalle: `Último pago ${formatearFecha(ciclo.fecha_ultimo_pago)}`,
    referencia: formatearReferenciaCiclo(ciclo)
  };
}
...
function renderizarListaEspera() {
  ...
  if (!listaEsperaGlobal.length) {
    contenedor.innerHTML = `<p class="waitlist-empty mb-0">Todavía no hay clientes en lista de espera.</p>`;
    return;
  }
      ...
      <div class="row g-3">
        <div class="col-12 col-xl-4">
          <div class="history-block h-100">
            <h6 class="mb-3">Próxima clase</h6>
            ${proximaClase ? `
              <div class="history-item">
                <div class="history-item-header">
                  <div>
        ...
        <div class="col-12 col-xl-4">
          <div class="history-block h-100">
            ...
            ${crearItemsHistorial(asistidas, { vacio: "Todavía no registra asistencias." })}
          </div>
        </div>
      </div>
    </div>
